    ...
    return (
        <>
            <p className='text-xl underline text-white text-center mx-5 px-5 my-5'>Your Bookmarks</p>
            {
                [...bookmarks].length > 0
                    ? bookmarks.map( ( quote ) => (
                        <div key={ quote._id } className='flex items-center justify-evenly bg-[#00000011] rounded-md mb-5'>
                            <QuoteCard content={ quote.content } author={ quote.author } />
                            <span onClick={ () => handleRemoveBookmark( quote._id ) } className='flex items-center text-lg ring-2 rounded cursor-pointer mx-5 py-0.5 px-3 shadow-sm shadow-white active:opacity-30 hover:opacity-60'>
